...
exports.run = function(bot, message, args) {
    ...
    for (let [channel, number] of usedChannelsMap) usedChannels += `<#${channel}> : **${number}** messages \`(${Math.round(10 * number * 100 / (message.member.statistics.types.chat.total + message.member.statistics.types.bots.total)) / 10}%)\`\n`;
    ...
    for (let [userBot, number] of usedBotsMap) usedBots += `<@${userBot}> : **${number}** messages \`(${Math.round(10 * number * 100 / botMessages) / 10}%)\`\n`;
